...
def _radius_search_warp(
    ...
    #print("flag222, for device check", print(device))
    ...
    torch_offset = torch.zeros(len(result_count) + 1, device=torch.device("cuda", device.ordinal), dtype=torch.int32)
...
def batched_radius_search_warp(
    ...
    device = wp.get_cuda_device(points.device.index)
